...
def leaderboard_screen():
    screen.fill(WHITE)
    background_img = load_back_image("rank2",WIDTH,HEIGHT)
    if background_img:
...
def initial_screen():
    ...
    while True:
        if background_img:
            screen.blit(background_img, (0, 0))
        draw_text("鱼了个鱼", chinese_font, BLACK, 280, 50)
        ...
        for event in pygame.event.get(): #(自动生成部分)
            if event.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
...
def game_over_screen():
    ...
    if victory:
        ...
        if not player_name.strip():
            player_name = "匿名"  # 默认用户名为“匿名” (自动生成部分)
